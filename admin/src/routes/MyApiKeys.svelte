...
<div class="p-8">
  <div class="page-header">
    <div class="flex items-center justify-between">
      <div>
        <h1 class="text-[28px] font-extrabold text-surface-800 tracking-tight">My API Keys</h1>
        <p class="text-surface-500 text-sm mt-1.5 font-medium">Manage your personal API keys</p>
      </div>
      <button
      ...
      <div class="overflow-x-auto">
        {#if apiKeys.length === 0}
          <div class="px-6 py-16 text-center">
            ...
            <p class="text-surface-500 text-sm font-medium">No API keys yet</p>
            <p class="text-surface-400 text-xs mt-1">Create your first API key to authenticate with the API</p>
          </div>
        {:else}
          <table class="w-full">
            <thead class="bg-surface-50 border-b border-surface-100">
              <tr>
                <th class="px-6 py-3 text-left text-xs font-semibold text-surface-500 uppercase tracking-wider">Name</th>
                <th class="px-6 py-3 text-left text-xs font-semibold text-surface-500 uppercase tracking-wider">Key Prefix</th>
                <th class="px-6 py-3 text-left text-xs font-semibold text-surface-500 uppercase tracking-wider">Status</th>
                <th class="px-6 py-3 text-left text-xs font-semibold text-surface-500 uppercase tracking-wider">Created</th>
                <th class="px-6 py-3 text-left text-xs font-semibold text-surface-500 uppercase tracking-wider">Expires</th>
            ...
            <tbody class="divide-y divide-surface-100">
              {#each apiKeys as key (key.id)}
                <tr class="hover:bg-surface-50 transition-colors">
                  <td class="px-6 py-4">
                    <p class="text-sm font-semibold text-surface-800">{key.name}</p>
                  </td>
                  <td class="px-6 py-4">
                    <code class="text-xs font-mono bg-surface-100 px-2 py-1 rounded text-surface-600">{key.key_id || key.key_prefix || key.id}</code>
                  </td>
                  <td class="px-6 py-4">
                  ...
                  <td class="px-6 py-4 text-right">
                    <button
                      ...
                      class="p-2 rounded-lg text-surface-400 hover:text-red-500 hover:bg-red-50 transition-all disabled:opacity-30 disabled:cursor-not-allowed"
                      title="Revoke"
                    >
...
<div class="fixed inset-0 bg-surface-900/50 backdrop-blur-sm flex items-center justify-center z-50 modal-overlay">
  <div class="bg-sky-50 rounded-2xl p-6 w-full max-w-md shadow-elevated-lg border border-indigo-200 modal-content">
    ...
    {#if newlyCreatedKey}
      <div class="mb-4 p-4 bg-emerald-50 border border-emerald-200 rounded-xl">
        ...
        <div class="flex items-center gap-2">
          <code class="flex-1 text-xs font-mono bg-white border border-emerald-200 rounded-lg px-3 py-2 text-surface-700 break-all">{newlyCreatedKey}</code>
          <button
    ...
    {:else}
      <div class="space-y-4">
        <div>
          <label for="myapikey-org" class="block text-xs font-semibold text-surface-500 uppercase tracking-wider mb-2">Organization</label>
          <select
            id="myapikey-org"
        ...
        <div>
          <label for="myapikey-name" class="block text-xs font-semibold text-surface-500 uppercase tracking-wider mb-2">Name</label>
          <input
            id="myapikey-name"
            type="text"
        ...
        <div>
          <label for="myapikey-expires" class="block text-xs font-semibold text-surface-500 uppercase tracking-wider mb-2">Expires In (days)</label>
          <input
            id="myapikey-expires"
            type="number"
