...
<div class="p-8">
  <div class="page-header">
    <div class="flex items-center justify-between">
      <div>
        <h1 class="text-[28px] font-extrabold text-surface-800 tracking-tight">Roles</h1>
        <p class="text-surface-500 text-sm mt-1.5 font-medium">Role definitions and permissions</p>
      </div>
    </div>
  </div>
  ...
  {:else}
    <div class="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
      {#each roles as role (role.id)}
        <div class="bg-sky-50 backdrop-blur-sm rounded-2xl border border-indigo-200/60 p-6 card">
          <div class="flex items-start gap-4">
            ...
            <div class="flex-1 min-w-0">
              <h3 class="text-surface-800 font-semibold text-[15px] truncate mb-0.5">{role.name}</h3>
          ...
          {#if role.description}
            <p class="mt-4 text-surface-600 text-sm">{role.description}</p>
          {/if}
          <div class="mt-4 pt-4 border-t border-surface-100">
            <p class="text-xs font-semibold text-surface-500 uppercase tracking-wider mb-2">Permissions</p>
            <div class="flex flex-wrap gap-1">
              ...
              {:else}
                ...
                {#if (role.permissions || []).length > 5}
                  <span class="px-2 py-1 rounded bg-surface-100 text-surface-600 text-xs">+{role.permissions.length - 5} more</span>
                {/if}
              {/if}
            </div>
          </div>
