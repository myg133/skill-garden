...
<div class="p-8">
  <div class="page-header">
    <div class="flex items-center justify-between">
      <div>
        <h1 class="text-[28px] font-extrabold text-surface-800 tracking-tight">Identities</h1>
        <p class="text-surface-500 text-sm mt-1.5 font-medium">Manage users, agents and system identities</p>
      </div>
      <button
      ...
      <table class="w-full">
        <thead class="bg-surface-50 border-b border-surface-200">
          <tr>
            ...
            <th class="px-6 py-4 text-left text-xs font-semibold text-surface-500 uppercase tracking-wider">Type</th>
            <th class="px-6 py-4 text-left text-xs font-semibold text-surface-500 uppercase tracking-wider">Email</th>
            <th class="px-6 py-4 text-left text-xs font-semibold text-surface-500 uppercase tracking-wider">Status</th>
            <th class="px-6 py-4 text-left text-xs font-semibold text-surface-500 uppercase tracking-wider">Created</th>
            ...
            <tr class="hover:bg-surface-50 transition-colors">
              <td class="px-6 py-4">
                <div class="flex items-center gap-3">
                  ...
                  <div>
                    <p class="text-sm font-semibold text-surface-800">{identity.name}</p>
                    <p class="text-xs text-surface-400 font-mono">{identity.id}</p>
                  </div>
                </div>
              </td>
              <td class="px-6 py-4">
              ...
              <td class="px-6 py-4 text-sm text-surface-600">{identity.email || '-'}</td>
              <td class="px-6 py-4">
                <span class="px-2.5 py-1 rounded-full text-xs font-medium {identity.status === 'active' ? 'bg-emerald-100 text-emerald-700' : 'bg-amber-100 text-amber-700'}">
                  {identity.status}
                </span>
              </td>
              <td class="px-6 py-4 text-sm text-surface-500">{new Date(identity.created_at).toLocaleDateString()}</td>
...
<div class="fixed inset-0 bg-surface-900/50 backdrop-blur-sm flex items-center justify-center z-50 modal-overlay">
  <div class="bg-sky-50 rounded-2xl p-6 w-full max-w-md shadow-elevated-lg border border-indigo-200 modal-content">
    <h2 class="text-lg font-bold text-surface-800 mb-5">Create Identity</h2>
    <div class="space-y-4">
      <div>
        <label for="identity-type" class="block text-xs font-semibold text-surface-500 uppercase tracking-wider mb-2">Type</label>
        <select
          id="identity-type"
      ...
      <div>
        <label for="identity-name" class="block text-xs font-semibold text-surface-500 uppercase tracking-wider mb-2">Name</label>
        <input
          id="identity-name"
          type="text"
      ...
      <div>
        <label for="identity-email" class="block text-xs font-semibold text-surface-500 uppercase tracking-wider mb-2">Email</label>
        <input
          id="identity-email"
          type="email"
      ...
      <div>
        <label for="identity-external-id" class="block text-xs font-semibold text-surface-500 uppercase tracking-wider mb-2">External ID (optional)</label>
        <input
