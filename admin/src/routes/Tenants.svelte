...
<div class="p-8">
  <div class="page-header">
    <div class="flex items-center justify-between">
      <div>
        <h1 class="text-[28px] font-extrabold text-surface-800 tracking-tight">Tenants</h1>
        <p class="text-surface-500 text-sm mt-1.5 font-medium">Manage tenant organizations and companies</p>
      </div>
      <button
  ...
  {:else}
    <div class="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
      {#each tenants as tenant (tenant.id)}
        <div class="group bg-sky-50 backdrop-blur-sm rounded-2xl border border-indigo-200/60 p-6 card card-interactive">
          <div class="flex items-start gap-4">
            <div class="w-11 h-11 rounded-xl bg-gradient-to-br from-emerald-500 to-teal-600 flex items-center justify-center font-bold text-lg shadow-glow flex-shrink-0 group-hover:scale-105 transition-transform duration-300">
              {tenant.name[0]?.toUpperCase() || '?'}
            </div>
            <div class="flex-1 min-w-0">
              <h3 class="text-surface-800 font-semibold text-[15px] truncate mb-0.5">{tenant.name}</h3>
              <p class="text-surface-400 text-xs font-mono truncate">{tenant.slug}</p>
            </div>
            <span class="px-2 py-1 rounded-full text-xs font-medium {tenant.status === 'active' ? 'bg-emerald-100 text-emerald-700' : 'bg-amber-100 text-amber-700'}">
              {tenant.status}
            </span>
          </div>
...
<div class="fixed inset-0 bg-surface-900/50 backdrop-blur-sm flex items-center justify-center z-50 modal-overlay">
  <div class="bg-sky-50 rounded-2xl p-6 w-full max-w-md shadow-elevated-lg border border-indigo-200 modal-content">
    <h2 class="text-lg font-bold text-surface-800 mb-5">Create Tenant</h2>
    <div class="space-y-4">
      <div>
        <label for="tenant-name" class="block text-xs font-semibold text-surface-500 uppercase tracking-wider mb-2">Name</label>
        <input
          id="tenant-name"
          type="text"
      ...
      <div>
        <label for="tenant-slug" class="block text-xs font-semibold text-surface-500 uppercase tracking-wider mb-2">Slug</label>
        <input
          id="tenant-slug"
          type="text"
