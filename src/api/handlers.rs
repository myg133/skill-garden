...
pub async fn list_identities_handler(
    ...
    let identities = if is_super {
        ...
        state
            .identity
            .list_by_tenants(&allowed, limit, offset)
            .await
    }
    .map_err(|e| ApiError::InternalError(e.to_string()))?;
    Ok((
...
pub async fn list_groups_handler(
    ...
    let groups = if is_super {
        ...
        state
            .group
            .list_by_org_tenants(&allowed, limit, offset)
            .await
    }
    .map_err(|e| ApiError::InternalError(e.to_string()))?;
...
pub async fn admin_login_handler(
    ...
    let token =
        crate::api::generate_token_full(&user.id.to_string(), Some(user.id), true, &["admin"], &[])
            .map_err(|e| ApiError::Unauthorized(format!("{:?}", e)))?;
...
pub async fn list_sessions_handler(
    ...
    let sessions = if is_super {
        ...
        state
            .session
            .list_by_org_tenants(&allowed, limit, offset, status)
            .await
    }
    .map_err(|e| ApiError::InternalError(e.to_string()))?;
