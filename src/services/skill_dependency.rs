...
use crate::db::repositories::session_context::{NewSkillDependency, SessionContextRepository};
...
impl SkillDependencyService {
    ...
    pub async fn parse_skill_dependencies(
        &self,
        skill: &ModelSkill,
    ) -> Result<Vec<SkillDependency>, AppError> {
        ...
        for dep_skill_id in &skill.dependencies {
            if !dependencies
                .iter()
                .any(|d| d.dependency_skill_id == *dep_skill_id)
            {
                dependencies.push(SkillDependency {
                    skill_id: skill.id.clone(),
    ...
    ) -> Result<Vec<ResolvedSkill>, AppError> {
        let resolved_ids = self
            .context_repo
            .resolve_dependencies(skill_ids)
            .await
            .map_err(|e| AppError::InternalError(e.to_string()))?;
    ...
    pub async fn get_skill_dependency_tree(
        &self,
        skill_id: &str,
    ) -> Result<DependencyTree, AppError> {
        let mut visited = HashMap::new();
        self.build_dependency_tree_impl(skill_id, &mut visited)
            .await
    }

    async fn build_dependency_tree_impl(
        &self,
        skill_id: &str,
        ...
        for dep in deps {
            if !visited.contains_key(&dep.dependency_skill_id) {
                ...
                let child_tree =
                    Box::pin(self.build_dependency_tree_impl(&dep.dependency_skill_id, visited))
                        .await?;
    ...
    pub async fn check_circular_dependency(&self, skill_id: &str) -> Result<bool, AppError> {
        ...
        self.find_cycle_impl(skill_id, &mut visited, &mut stack)
            .await
    }

    async fn find_cycle_impl(
        &self,
        skill_id: &str,
        visited: &mut HashMap<String, bool>,
    ...
    pub async fn get_missing_dependencies(&self, skill_id: &str) -> Result<Vec<String>, AppError> {
        ...
        for dep in deps {
            if !dep.is_optional {
                let skill = self
                    .skill_repo
                    .find_by_id(&dep.dependency_skill_id)
                    .await
                    .map_err(|e| AppError::InternalError(e.to_string()))?;
    ...
    pub async fn delete_skill_dependencies(&self, skill_id: &str) -> Result<(), AppError> {
        self.context_repo
            .delete_skill_dependencies(skill_id)
            .await
            .map_err(|e| AppError::InternalError(e.to_string()))
    }
}
